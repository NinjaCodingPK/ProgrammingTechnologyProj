...
/**
 * Method adds a qualification of developers and their count which were set by 
 * client to current project.
 * 
 * <p>Method redirects user back to the web-page for setting up a new project.
 * @author wookie
 */
public class SubmitProjectQualificationCommand extends Command {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response)
        ...
        serviceFactory.getProjectQualificationService().addProjectQualification(projectId,
                                            extractor.extractInt(request, Attributes.QUALIFICATION_ID),
                                            extractor.extractInt(request,Attributes.COUNT));
        
        request.setAttribute(Attributes.TASK_LIST, serviceFactory.getTaskService().getByProject(projectId));
